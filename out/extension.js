...
var forms = new Array();
var panel;
function renderBindFun(binding) {
}
let cmds = new Array();
function renderWinContent(window, dic, title) {
    var WinContent = "#!/usr/bin/python3\n\
...
function renderBtnContent(inputs, dic) {
    var Content = "";
...
function renderEtrContent(inputs, dic) {
    var Content = "";
    for (var i = 0; i < inputs.length; i++) {
        let eleid = inputs[i].eleid;
        Content +=
...
function renderLabContent(inputs, dic) {
    var Content = "";
    for (var i = 0; i < inputs.length; i++) {
        let eleid = inputs[i].eleid;
        Content +=
...
function renderFrmContent(inputs, dic) {
    var Content = "";
    for (var i = 0; i < inputs.length; i++) {
        let eleid = inputs[i].eleid;
        Content +=
...
function renderCkbContent(inputs, dic) {
    var Content = "";
    for (var i = 0; i < inputs.length; i++) {
        let eleid = inputs[i].eleid;
        Content +=
...
function renderRdbContent(inputs, dic) {
    var Content = "";
    for (var i = 0; i < inputs.length; i++) {
        let eleid = inputs[i].eleid;
        Content +=
...
function renderPgbContent(inputs, dic) {
    var Content = "";
    for (var i = 0; i < inputs.length; i++) {
        let eleid = inputs[i].eleid;
        Content +=
...
function renderFdgContent(inputs, dic) {
    var Content = "";
    for (var i = 0; i < inputs.length; i++) {
        let eleid = inputs[i].eleid;
        let cmd;
...
function renderCgStyFun() {
    let content = "	def change_style(self,style_name,foreground,background):\n\
...
function activate(context) {
    ...
    let itemClick = vscode.commands.registerCommand('itemClick', (label) => {
        if (label === "new form") {
            ...
            panel = vscode.window.createWebviewPanel('pybuilder', title, vscode.ViewColumn.One, {
                enableScripts: true,
                retainContextWhenHidden: true,
            });
            panel.webview.html = getWebViewContent(context, "src/static/html/pybuilder.html");
            panel === null || panel === void 0 ? void 0 : panel.webview.postMessage({ 'txt': title });
            panel === null || panel === void 0 ? void 0 : panel.webview.onDidReceiveMessage(message => {
                console.log(message);
        ...
        panel === null || panel === void 0 ? void 0 : panel.webview.postMessage({ 'txt': label });
        if (label === "submit") {
            label = "arrow";
    ...
    TreeViewProvider_1.ElementProvider.initElementItem();
    TreeViewProvider_1.SysChoiceProvider.initSysChoiceItem();
...
function getWebViewContent(context, templatePath) {
    ...
    const staticPath = vscode.Uri.file(path_1.resolve(basePath)).with({ scheme: 'vscode-resource' }).toString() + '"';
